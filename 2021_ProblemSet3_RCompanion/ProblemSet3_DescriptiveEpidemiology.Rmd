...
Before you get started on this companion, you should have already reviewed the "Introduction to Data Analysis Companions" file and the "0_Getting Started" project in the PH250A workspace on RStudio Cloud (via Datahub). These files will provide you with a brief orientation to RStudio Cloud as well as Datahhub. They also include instructions about how to submit the assignment for extra credit, if you choose to do so.
...
# First let's load the packages we'll need for this exercise. R comes with a number of built in commands for different functions. There are also external packages that we can install to execute different commands. To use an external package, we must first ask R to install it. We only have to install it once. After that, we must load the package using the "library" command each time we start a new session and want to use it. Note that to install a package, we must put quotes around it's name. To load a package, we do not use quotes.
...
Need to determine how it looks on Datahub
#####################################################

```{r}
#This code sets the working directory. It tells R where to find the data and how to import it. Here, we're telling R that our working directory is "/cloud/project" and that the data are stored in comma-separated file (CSV) format, which means the individual data values are separated by commas.
...
*Can you guess what the square brackets in the code above do? Does the first value before the comma indicate the rows or columns you wish to select?*
...
*Did you get the same answer that you computed in Problem 2B by hand? What does the value in the first column of the table above represent?*
...
### Problem 3: ASleep duration and high blood pressure
...
To answer the attributable proportion questions in parts #3 (A), (B), and (C), let's generate a similar table as we did for Problem #2. 
...
Now let's consider problem #3D. Here you are being asked about the attributable proportion in the total population for high blood pressure due to short sleep duration **in California** - a different population than the previous question. Here we can use R as a calculator using the APe from problem #3(A) (recall that APt = APe * Prevalence (exposure)|Disease. 
...
Now that you've worked through problems #2 and #3 on the problem set, you're ready to conduct your own epidemiologic analysis. You will use the CS command you just learned to investigate the relationship between sleep duration and diabetes in the NHANES sample population. 
...
Congratulations, you've completed the first data analysis companion! You can return to learn more R in Problem Set #4 on Experimental Studies. 
...
2. Modify code as directed in the sections titled  “ACTIVITY”. This might mean modifying a calculation or changing the look of a graph. (Note that when you access a project created by someone else, RStudio Cloud automatically creates a temporary copy of the project for you. You can play with and make edits to it, but none of your changes will be reflected in the original. 

############# Check if this is TRUE
If you’d like to keep your changes, save a copy of the project for yourself by pressing the ‘Save a Permanent Copy’ button.)
##################
...
5. Save the resulting Word document as follows: PH250A_PS3_KSO. Replace the ‘3’ with the number of the data analysis companion (3, 4, 6, or 10), and replace KSO with your own initials. 
