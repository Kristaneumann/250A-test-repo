...
# PH250A: GUIDE TO R DATA ANALYSIS COMPANIONS 

This is the introduction to the four **optional** data analysis R companions to P250A Problem Sets. These companions are intended to be fun and inspire you to be curious about epidemiological data. We'll show you how epidemiologists perform some of the calculations you learn about it class as well as how to explore data on your own. Of course, make sure you understand how to do the problem sets manually (on paper) _first_ since that's what you'll be asked to do on exams. 

No previous programming experience? No problem - you're in the right place. These companions are intended for people without any experience with statistical software or programming. **Our goal isn't to teach you how to code in R**; rather, we hope to demystify how actual epidemiologists and statisticians conduct analyses. In other classes (and at places like the [D-Lab](http://dlab.berkeley.edu/) you will learn the details about coding on your own. For now, just follow along and have fun!
...
In this class, we will be using **Datahub**, which is a system that allows us to access **R Studio Cloud**. It also makes it easy for us to share all of the program and data files you will need (without having to download them). **Datahub** is maintained by UC Berkeley’s Data Science Division. There is no need to “create an account”; by default, all users with a berkeley.edu email have access to **Datahub**. 

When you click a **Datahub link** for the first time, you will be redirected to sign in with CalCentral, then taken to a page to authorize **Datahub** for usage with bCourses. When you confirm this, your user account is automatically generated on **Datahub**, and you are then redirected to your online workspace. 


### R Scripts vs R Markdown files
All R files allow us to write scripts or lines of code to be executed/run. You can create a new file for R code by going to File>New File>R Script. **R Markdown** files allow us to easily define *code chunks* or lines of code that can be run all at once. **R Markdown files** also allow us to easily create documents (Word, PDF, or HTML) that include code, output, and graphics all together. In this class, we'll be using **R Markdown** files like this one to work through the problem sets. 
...
We can use R like a simple calculator, for data management and manipulation, for data analysis, and for generating graphics.
...
(Note that this code did not print because of the “include=FALSE” clause at the top.)

4. A prompt will appear: “R Markdown Render Completed.” Select "Download File" to download the Word document. This document will also appear as a link in the bottom right pane.
...
5. Save the resulting Word document as follows: PH250A_PS2_KSO. Replace the ‘2’ with the number of the data analysis companion (2, 4, 6, or 10), and replace KSO with your own initials. 
